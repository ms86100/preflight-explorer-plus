...
const mockMaybeSingleResponse = () => Promise.resolve({ data: null, error: null });
const mockOrderLimitResponse = () => Promise.resolve({ data: [], error: null });
...
const mockInvokeResponse = () => Promise.resolve({ data: { success: true }, error: null });

const mockLimitFn = vi.fn(mockOrderLimitResponse);
const mockEqOrderFn = vi.fn(() => ({ limit: mockLimitFn }));
...
vi.mock('@/integrations/supabase/client', () => ({
  supabase: {
    ...
    functions: {
      invoke: vi.fn(mockInvokeResponse),
    },
  },
}));
  ...
  describe('LdapConfiguration', () => {
    ...
    it('should support all sync statuses', () => {
      ...
      statuses.forEach((status) => {
        const config: LdapConfiguration = {
          id: 'config-uuid', name: 'Test', server_url: 'ldap://test', port: 389,
          base_dn: 'dc=test', bind_dn: null, use_ssl: false, search_filter: null,
          user_id_attribute: 'uid', email_attribute: 'mail', display_name_attribute: 'cn',
          group_base_dn: null, group_search_filter: null, sync_interval_minutes: 60,
          is_active: true, last_sync_at: null, last_sync_status: status,
        };
  ...
  describe('LdapGroupMapping', () => {
    it('should support role target type', () => {
      const mapping: LdapGroupMapping = {
        id: 'mapping-uuid', ldap_config_id: 'config-uuid',
        ldap_group_dn: 'cn=admins,ou=groups,dc=example,dc=com', ldap_group_name: 'admins',
        target_type: 'role', target_role: 'admin', target_group_id: null,
        target_project_role_id: null, is_active: true,
      };
    ...
    it('should support group target type', () => {
      const mapping: LdapGroupMapping = {
        id: 'mapping-uuid', ldap_config_id: 'config-uuid',
        ldap_group_dn: 'cn=developers,ou=groups,dc=example,dc=com', ldap_group_name: 'developers',
        target_type: 'group', target_role: null, target_group_id: 'group-uuid',
        target_project_role_id: null, is_active: true,
      };
  ...
  describe('LdapSyncLog', () => {
    it('should represent successful sync', () => {
      const log: LdapSyncLog = {
        id: 'log-uuid', ldap_config_id: 'config-uuid', sync_type: 'full',
        status: 'success', started_at: '2024-01-15T10:00:00Z',
        completed_at: '2024-01-15T10:05:00Z', users_synced: 150,
        groups_synced: 10, roles_assigned: 25, roles_revoked: 5, errors: null,
      };
  ...
  describe('SyncResult', () => {
    it('should represent successful sync result', () => {
      const result: SyncResult = {
        success: true, users_synced: 100, groups_synced: 5,
        roles_assigned: 20, roles_revoked: 3,
      };
...
function validateServerUrl(url: string): { valid: boolean; error?: string } {
  ...
  if (!pattern.test(url)) return { valid: false, error: 'Invalid LDAP URL format. Use ldap:// or ldaps://' };
  return { valid: true };
}

function validateDN(dn: string): { valid: boolean; error?: string } {
  if (!dn) return { valid: false, error: 'Distinguished Name is required' };
  const dnPattern = /^([a-zA-Z]+=.+,)*[a-zA-Z]+=.+$/;
  if (!dnPattern.test(dn)) return { valid: false, error: 'Invalid DN format (e.g., cn=admin,dc=example,dc=com)' };
  return { valid: true };
}

function validateFilter(filter: string): { valid: boolean; error?: string } {
  if (!filter) return { valid: true };
  if (!filter.startsWith('(') || !filter.endsWith(')')) return { valid: false, error: 'Filter must be enclosed in parentheses' };
  let depth = 0;
  for (const char of filter) {
    if (char === '(') depth++;
    if (char === ')') depth--;
    if (depth < 0) return { valid: false, error: 'Unbalanced parentheses in filter' };
  }
  if (depth !== 0) return { valid: false, error: 'Unbalanced parentheses in filter' };
  return { valid: true };
}
...
function getSyncHealth(config: LdapConfiguration): 'healthy' | 'warning' | 'error' | 'unknown' {
  ...
  if (now.getTime() - lastSync.getTime() > overdueMs) return 'warning';
  return 'healthy';
}
...
describe('LDAP Validation', () => {
  describe('validateServerUrl', () => {
    ...
    it('should reject invalid URLs', () => {
      expect(validateServerUrl('')).toEqual({ valid: false, error: 'Server URL is required' });
      expect(validateServerUrl('http://ldap.example.com').valid).toBe(false);
    });
  });
  ...
  describe('validateDN', () => {
    ...
    it('should reject invalid DNs', () => {
      expect(validateDN('').valid).toBe(false);
      expect(validateDN('invalid').valid).toBe(false);
    });
  });
...
describe('LDAP Helpers', () => {
  describe('getTimeSinceSync', () => {
    ...
    it('should return Just now for recent sync', () => {
      expect(getTimeSinceSync(new Date().toISOString())).toBe('Just now');
    });
  });
  ...
  describe('getSyncHealth', () => {
    it('should return unknown for inactive config', () => {
      const config: LdapConfiguration = {
        ...
        user_id_attribute: 'uid', email_attribute: 'mail', display_name_attribute: 'cn',
        group_base_dn: null, group_search_filter: null, sync_interval_minutes: 60,
        is_active: false, last_sync_at: null, last_sync_status: null,
      };
